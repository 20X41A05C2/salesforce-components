...
export default class DataTableOfParticularRecordTask extends LightningElement {
    @track hospitalData;
    @track doctorData;
    @track showHospitalTable=true;
    @track showDoctorTable=false;
    ...
    @track doctorColumns=[
        ...
        {lbel:'Email',fieldName:'Prescriber_Mail_ID__c'}
    ]
    ...
    handleHospitalRowAction(event) {
        ...
        console.log('Name::'+this.selectedHospitalName);
    ...
    handleBack(event){
        ...
        this.showHospitalTable=true;
    }

    fetchDoctors(){
        getDoctorsByHospital({hospitalId:this.selectedHospitalId,startDate:this.startDate,endDate:this.endDate})
        .then(result=>{
            this.doctorData=result;
            this.showDoctorTable=true;
            this.showHospitalTable=false;
        })
        .catch(error=>{
            this.showToast('Error',error.body.message,'error');
        })
